...
RUN apt update && apt install -y \
        ...
        git \
        jq && \
    python -m pip install --upgrade pip huggingface_hub numpy==1.26.0 && \
...
RUN mkdir -p local_data models/cache .config/matplotlib && \
    ...
    /home/worker/app/.venv/bin/pip install --force-reinstall --no-cache-dir llama-cpp-python numpy==1.26.0 && \
